...
class imagenesController {
    ...
    async subirImagen(tabla, idCampo, id, imagenBase64) {
        try {
            // Verificar si el registro existe
            const [registro] = await db.query(`SELECT * FROM ?? WHERE ?? = ?`, [tabla, campoId, id]);

            if (registro.length === 0) {
                return { error: 'Registro no encontrado con el ID proporcionado' };
            }

            // Convertir la imagen de base64 a binario
            const imagenBuffer = Buffer.from(imagenBase64, 'base64');

            //Ejecutar la consulta para actualizar o insertar la imagen en la tabla correspondiente
    ...
    // Eliminar la imagen (lmpiar el campo de imagen en la base de datos)
    async eliminarImagen(tabla, campoId, id) {
        try {
            ...
            // Ejecutar la consulta para eliminar la imagen (establecer a NULL)
    ...
    async procesarImagen(tabla, campoId, id, imagenBase64 = null) {
        if (imagenBase64) {
            // Si se proporciona una imagen en base64, la subimos o actualizamos
            return await this.subirImagen(tabla, campoId, id, imagenBase64);
        } else {
            // Si no se proporciona imagen, intentamos obtenerla
